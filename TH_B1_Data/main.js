...
var showCart = function () {
  ...
  hoadon.forEach(function (sp) {
    if (sp.count > 0) {
      ...
      td_count.innerHTML = `<span>${sp.count}</span>`;
      tr.appendChild(td_count);

      var td_price = document.createElement("td");
      td_price.innerHTML = `<span>${sp.price}</span>`;
      tr.appendChild(td_price);

      var td_total = document.createElement("td");
      td_total.innerHTML = `<span>${sp.total}</span>`;
  ...
  trA.innerText = `Thành tiền (A) = ${totalPreTax} đ`;
  var trB = document.getElementById("trB");
  trB.innerText = `Chiết Khấu (B) = 0.1xA = ${discount} đ`;
  var trC = document.getElementById("trC");
  trC.innerText = `Thuế (C) = 10% x (A-B) = ${tax} đ`;
  var trD = document.getElementById("trD");
  trD.innerText = `Tổng cộng (D) = A-B+C = ${finalTotal} đ`;
};
